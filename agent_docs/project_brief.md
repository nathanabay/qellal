...
- **Architecture rules:** business logic in `lib/`, not in page components; scrapers isolated in `/scrapers` and only ever write `pending_review` rows; single `canAccess()` gate for future payments.
